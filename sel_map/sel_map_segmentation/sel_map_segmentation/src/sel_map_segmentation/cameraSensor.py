...
class CameraSensor():
	def __init__(self):
		...
		self.onGPU			 	= rospy.get_param("semseg/ongpu_projection", False)
		...
		if self.labels > 0:
			# print("!!self.labels > 0", self.labels)
			# print("!!self.onGPU is:", self.onGPU)
			# self.onGPU = False
			# print("!!change self.onGPU is:", self.onGPU)
	...
	def projectMeasurementsIntoSensorFrame_torch(self, intrinsic=None, R=None, min_depth=0, max_depth=5.0):
		with torch.no_grad():
			...
			p3d = torch.ones(3,mask.shape[0]).to(self.network.device)			# pin_memory is used for CUDA
			p3d[0,:] = u_coord[mask]
			p3d[1,:] = v_coord[mask]
